...
<div class="max-w-7xl mx-auto mt-6">
    ...
    <div class="flex items-center justify-between gap-4 mb-3">
        <div>
            ...
            <p class="text-sm text-slate-500 font-light mt-1">
                Data pekerjaan ayah/ibu bisa diedit inline.
            </p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-4">
        ...
        <form id="filterForm" method="GET" action="{{ route('orangtua.pekerjaan') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
                ...
                {{-- SEARCH PEKERJAAN --}}
                <div>
                    <label class="text-slate-600 text-xs font-extralight">Pekerjaan</label>
                    <input type="text"
                        id="searchPekerjaan"
                        name="search_pekerjaan"
                        value="{{ request('search_pekerjaan') }}"
                ...
                {{-- RESET --}}
                <div>
                    <a href="{{ route('orangtua.pekerjaan') }}"
            ...
            <table class="min-w-full text-xs table-fixed">
                <thead class="bg-slate-200 text-slate-600">
                    <tr class="text-left">
                        ...
                        <th class="px-2 py-1 font-normal">Pekerjaan Ayah</th>
                        <th class="px-2 py-1 font-normal">Pekerjaan Ibu</th>
                ...
                <tbody class="divide-y divide-slate-100 text-xs">
                    ...
                    <tr data-id="{{ $o->id }}" class="hover:bg-slate-50/60 transition">
                        ...
                        <td class="px-2 py-1 pekerjaan_ayah text-slate-700">
                            {{ $o->pekerjaan_ayah ?? '-' }}
                        </td>

                        <td class="px-2 py-1 pekerjaan_ibu text-slate-700">
                            {{ $o->pekerjaan_ibu ?? '-' }}
                        </td>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const searchPekerjaan = document.getElementById('searchPekerjaan');
        ...
        if (searchPekerjaan) searchPekerjaan.addEventListener('input', liveSubmit);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.editBtn').forEach(button => {
            ...
            button.addEventListener('click', function() {
                ...
                const ayahCell = row.querySelector('.pekerjaan_ayah');
                const ibuCell = row.querySelector('.pekerjaan_ibu');

                if (this.textContent.trim() === 'Edit') {

                    const ayahVal = ayahCell.textContent.trim() === '-' ? '' : ayahCell.textContent.trim();
                    const ibuVal = ibuCell.textContent.trim() === '-' ? '' : ibuCell.textContent.trim();
                ...
                `;
                    ...
                    const pekerjaan_ayah = ayahCell.querySelector('input').value;
                    const pekerjaan_ibu = ibuCell.querySelector('input').value;

                    fetch(`/orangtua/${id}/update-pekerjaan`, {
                            method: 'POST',
                            headers: {
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                ...
                                ayahCell.textContent = pekerjaan_ayah || '-';
                                ibuCell.textContent = pekerjaan_ibu || '-';
